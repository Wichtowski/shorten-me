...
export class UrlService {
  ...
  static async getUserUrls(user_id: string): Promise<UrlDocument[]> {
    await getMongoCluster();
    const urls = await Url.find({ user_id }).sort({ created_at: -1 }).lean().exec();
  ...
  static async incrementClicks(urlDoc: UrlDocument): Promise<void> {
    try {
      ...
      if (urlDoc.user_id === "anonymous") {
        await AnonymousUsage.findOneAndUpdate({ id: urlDoc.id }, { $inc: { clicks: 1 } });
      } else {
        await Url.findOneAndUpdate({ id: urlDoc.id }, { $inc: { clicks: 1 } });
      }
    } catch (error) {
